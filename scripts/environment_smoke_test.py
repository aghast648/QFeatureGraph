#!/usr/bin/env python3
"""Check QFeatureGraph's minimum supported Python environment.
...
core ``project.dependencies``, imports those dependencies and QFeatureGraph,
and performs a tiny in-memory feature-graph operation. It does not download a
dataset, train a model, or check optional development and quantum dependencies.
...
def run_tiny_feature_graph_check(imported_modules: dict[str, object]) -> bool:
    """Exercise a small, deterministic path through the core numerical stack."""
    numpy_module = imported_modules.get("numpy")
    networkx_module = imported_modules.get("networkx")
    torch_module = imported_modules.get("torch")
    if numpy_module is None or networkx_module is None or torch_module is None:
        return False

    try:
        activations = torch_module.tensor(
            [[1.0, 2.0, 3.0], [1.0, 2.0, 4.0]], dtype=torch_module.float32
        )
        correlation = float(numpy_module.corrcoef(activations.numpy())[0, 1])
        graph = networkx_module.Graph()
        graph.add_nodes_from((0, 1))
        graph.add_edge(0, 1, weight=max(0.0, correlation))

        assert activations.shape == (2, 3)
        assert bool(numpy_module.isfinite(correlation))
        assert 0.0 <= graph[0][1]["weight"] <= 1.0
    except Exception as exc:
        print(f"[FAIL] tiny feature-graph operation: {type(exc).__name__}: {exc}")
        return False

    print("[PASS] tiny feature-graph operation")
    return True


def main() -> int:
    print("QFeatureGraph environment smoke test")
    ...
    for package_name in dependencies:
        ...
        try:
            imported_modules[package_name.lower()] = importlib.import_module(module_name)
    ...
    try:
        qfeaturegraph = importlib.import_module("qfeaturegraph")
    except Exception as exc:
        print(f"[FAIL] import qfeaturegraph: {type(exc).__name__}: {exc}")
        checks.append(False)
    else:
        print(
            "[PASS] import qfeaturegraph: "
            f"{getattr(qfeaturegraph, '__version__', 'unknown')}"
        )
        checks.append(True)

    checks.append(run_tiny_feature_graph_check(imported_modules))
    ...
    print("Install or update QFeatureGraph and its core dependencies with:")
